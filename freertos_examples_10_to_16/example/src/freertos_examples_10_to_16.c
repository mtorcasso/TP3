...
#define EXAMPLE_17 (17) 	//Ejercicio 1
#define EXAMPLE_18 (18) 	//Ejercicio 2
#define EXAMPLE_19 (19)		//Ejercicio 3


#define TEST (EXAMPLE_19)
...
static void vHandlerTask(void *pvParameters)
{
	...
	while (1) {
		...
		/* To get here the event must have occurred.  Process the event (in this
		 * case we just print out a message). */
		DEBUGOUT("Handler task - Encolando dato\r\n");
...
int main(void)
{
	...
	xQueue = xQueueCreate(5, sizeof(long)); //inicializo cola vacia
    /* Before a semaphore is used it must be explicitly created.  In this example
     * a counting semaphore is created.  The semaphore is created to have a maximum
     * count value of 10, and an initial count value of 0. */
    xCountingSemaphore = xSemaphoreCreateCounting(10, 0);
	...
	if (xCountingSemaphore != NULL)
	{
		...
		priority of 3 is chosen. */
        xTaskCreate(vHandlerTask, (char *) "Handler", configMINIMAL_STACK_SIZE,(void *) 100,
        			(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
...
#if (TEST == EXAMPLE_18)		/* Using a binary semaphore to synchronize a task with an interrupt */
	...
	long lReceivedValue;
		...
		while (1) {
			...
			if (xStatus == pdPASS) {
				/* Data was successfully received from the queue, print out the received
				 * value. */
				DEBUGOUT("Tarea 2, dando semaforo tarea 3\r\n");
...
static void vReceiverTask(void *pvParameters)
{
	long lReceivedValue= (long) pvParameters;
...
void vSoftwareInterruptHandler(void)
{
	...
	xQueueSendToBackFromISR(xQueue, (void *) 100, &xHigherPriorityTaskWoken);
    /* Clear the software interrupt bit using the interrupt controllers
     * Clear Pending register. */
    mainCLEAR_INTERRUPT();
...
int main(void)
{
	...
	if (xCountingSemaphore != NULL)
	{
    	...
    	/* Creo tarea receptora*/
    	xTaskCreate(vReceiverTask, (char *) "vReceiver", configMINIMAL_STACK_SIZE, (void*) 100,
    						(tskIDLE_PRIORITY + 3UL), (xTaskHandle *) NULL);
...
#if (TEST == EXAMPLE_19)		/* Re-writing vPrintString() to use a semaphore */
...
static void vTask1( void *pvParameters )
{
	 ...
	 * case standard out. */
		...
		/* The following line will only execute once the semaphore has been
		 * successfully obtained - so standard out can be accessed freely. */
		DEBUGOUT("Tarea 1 ejecutandose--------------------------------------------------------\r\n");
		Board_LED_Set(LED3, LED_ON);
		uint32_t i= (204000000/100);
		while (i--);
		Board_LED_Set(LED3, LED_OFF);
		i= (204000000/100);
		while (i--);
...
int main(void)
{
	...
	if (xMutex != NULL) {
		 ...
		 * string they attempt to write is passed in as the task parameter.  The tasks
		 * are created at different priorities so some pre-emption will occur. */
		xTaskCreate(vTask1, (char *) "Print1", configMINIMAL_STACK_SIZE, NULL,
					(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
		xTaskCreate(vTask2, (char *) "Print2", configMINIMAL_STACK_SIZE, NULL,
							(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
		xTaskCreate(vTask3, (char *) "Print3", configMINIMAL_STACK_SIZE, NULL,
							(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
